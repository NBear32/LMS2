  ...
  lectureList.forEach((data) => {
    ...
    axios.get(urlCurrent)
      .then((response) => {
        ...
        if (response.data.userId === data.course_registration.user.userId && (data.course_registration.lectureStatus == "I")) {

          const datacr = data.course_registration;
          ...
          // 태그속성추가
          img.src = datacr.lecture.imagePath;
          lectureName.textContent = datacr.lecture.lectureName;
          lectureDate.textContent = datacr.lecture.educationPeriodStartDate + " ~ " + datacr.lecture.educationPeriodEndDate;
          lectureProgress.textContent = (datacr.progressLectureContentsSeq / datacr.finalLectureContentsSeq * 100).toFixed(1) + "%";
          progressBar2.style.width = (datacr.progressLectureContentsSeq / datacr.finalLectureContentsSeq * 100).toFixed(0) + "%";
